...
if __name__ == '__main__':
    ...
    sorted_ev_scan[0]=sorted_ev_scan[0]/100

    popt, pcov = curve_fit(poly_model, sorted_ev_scan[0][1:28],sorted_ev_scan[1][1:28])
    popt_v1, pcov_v1 = curve_fit(poly_model, sorted_ev_scan[0][1:28],sorted_ev_scan[2][1:28])
    poptv2, pcovv2 = curve_fit(poly_model, sorted_ev_scan[0][1:28],sorted_ev_scan[3][1:28])
    # Plot the original data
    plt.subplot(121)
    plt.scatter(sorted_ev_scan[0][1:28], sorted_ev_scan[1][1:28], label='g0 data')
    plt.scatter(sorted_ev_scan[0][1:28], sorted_ev_scan[2][1:28], label='v1 data')
    plt.scatter(sorted_ev_scan[0][1:28], sorted_ev_scan[3][1:28], label='v2 data')
    data_g0 = modeling(sorted_ev_scan[0][1:28],poly_model,popt)
    data_v1 = modeling(sorted_ev_scan[0][1:28],poly_model,popt_v1)
    data_v2 = modeling(sorted_ev_scan[0][1:28],poly_model,poptv2)
    ...
    data_d1 = modeling(sorted_ev_scan[0][4:28],poly_model,np.flip(diab_coeffs[0]))
    data_d2 = modeling(sorted_ev_scan[0][4:28],poly_model,np.flip(diab_coeffs[1]))
    data_coupling = modeling(sorted_ev_scan[0][4:28],poly_model,np.flip(diab_coeffs[2]))
